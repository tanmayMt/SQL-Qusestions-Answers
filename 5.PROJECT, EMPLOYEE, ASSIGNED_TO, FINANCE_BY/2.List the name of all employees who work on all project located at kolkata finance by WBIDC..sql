...
select e_name as Project_at_Kolkata 
from EMPLOYEE
where e_no in(
              select e_no
              from ASSIGNED_TO
              where p_no in (
                              ...
                              and p_no in(
                                           select p_no
                                           from FINANCE_BY
                                           where company_name='WBIDC'
                                          )
                            )
              )
              
select e_name as Project_at_Kolkata 
from PROJECT,ASSIGNED_TO,FINANCE_BY,EMPLOYEE
where PROJECT.p_no=FINANCE_BY.p_no=ASSIGNED_TO.p_no
and ASSIGNED_TO.e_no=EMPLOYEE.e_no
and location='Kolkata'
and company_name='WBIDC'